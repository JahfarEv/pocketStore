...
//   // Close the chat window and reset the conversation back to the welcome message
//   const handleClose = () => {
//     if (isListening) {
//       recognitionRef.current?.stop();
//     }
...
//               </div>
//             </div>
//             <button onClick={handleClose} className="p-1 -m-1 shrink-0">
//               <X />
//             </button>
//           </div>
...
//                       className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-medium pl-3 pr-3.5 py-1.5 transition shadow-sm"
//                     >
...
export default function PoketStorChat() {
  ...
  const [location, setLocation] = useState(null);
  const [isListening, setIsListening] = useState(false);
  const [selectedLang, setSelectedLang] = useState("en-US"); // New language state
  ...
  const toggleListening = () => {
    ...
    if (isListening) {
      ...
      setInput("");
      // Update the language dynamically right before starting
          ...
          <div className="border-t bg-white flex flex-col shrink-0">
            ...
            <div className="px-3 pt-2 pb-1 flex items-center justify-end gap-1.5 border-b border-gray-100">
              ...
              <select
                ...
                disabled={isListening}
                className="text-xs font-medium text-gray-600 bg-transparent border-0 outline-none cursor-pointer focus:ring-0"
              >
                {LANGUAGES.map((lang) => (
